...
class RSTDataset(Dataset):
    def __init__(
        self,
        rst_path,
        model_output_path,  # gpt生成的假设数据
        model_output_emb_path,  # 假设的embedding
        exp_text_path,  # 解释文本
        lexical_chains_dir,
        embeddings_dir,
        ...
        self.exp_text_path = exp_text_path
        ...
        with open(self.exp_text_path, "r") as f:
            self.exp_text = json.load(f)
        # 存储提前构建好的图，避免在训练时再构建
        self.graph_pairs = []
    ...
    def build_graphs(
        self, lexical_chains, embeddings, hyp_emb, exp_text, start_idx, end_idx
    ):
        """
        根据加载的词汇链和嵌入构建图。
        """
    ...
    def __getitem__(self, idx):
        g_premise, g_hypothesis, lexical_chain, hyp_emb_three, exp_text_three = (
            self.graph_pairs[idx]
        )
        return g_premise, g_hypothesis, lexical_chain, hyp_emb_three, exp_text_three
...
class NLIExpDataset(Dataset):
    def __init__(self, premises1, premises2, merged_premises, hypotheses, explanations):
        self.premises1 = premises1
        self.premises2 = premises2
        self.merged_premises = merged_premises
